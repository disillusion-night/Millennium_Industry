...
void main() {
    ...
    if (uv.x == posUV1.x) {
        ...
        float time = GameTime;
        time -= fract(dot(round(vertexColor.rgb * 255.0), vec3(65536.0, 256.0, 1.0)) / 24000.0);
        time *= 1200.0;
        uint ux = floatBitsToUint(cot);
        uint uy = floatBitsToUint(time);
        vec3 c;
    ...
    } else if (uv == vec2(baseUV.x - 1.0, baseUV.y) && marker > 0.5) {  // 开关标识
