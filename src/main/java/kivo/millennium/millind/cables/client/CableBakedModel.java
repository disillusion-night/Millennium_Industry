package kivo.millennium.millind.cables.client;

import kivo.millennium.millind.cables.ConnectorType;
import kivo.millennium.millind.cables.blocks.AbstractPipeBL;
...
import static kivo.millennium.millind.cables.client.CablePatterns.*;
import static kivo.millennium.millind.cables.client.CablePatterns.SpriteIdx.*;
...
public class CableBakedModel implements IDynamicBakedModel {
    ...
    private TextureAtlasSprite getTexture(String path) {
        return Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(getRL(path));
    }
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @NotNull RandomSource rand, @NotNull ModelData extraData, @Nullable RenderType layer) {
        ...
        if (side == null && (layer == null || layer.equals(RenderType.solid()))) {
            ...
            if (state != null) {
                north = state.getValue(AbstractPipeBL.NORTH);
                south = state.getValue(AbstractPipeBL.SOUTH);
                west = state.getValue(AbstractPipeBL.WEST);
                east = state.getValue(AbstractPipeBL.EAST);
                up = state.getValue(AbstractPipeBL.UP);
                down = state.getValue(AbstractPipeBL.DOWN);
            } else {
        ...
        // (FACADEID)
        BlockState facadeId = extraData.get(AbstractPipeBL.FACADEID);
        if (facadeId != null) {
