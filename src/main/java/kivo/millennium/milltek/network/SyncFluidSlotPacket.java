...
public class SyncFluidSlotPacket {
  ...
  public void toBytes(FriendlyByteBuf buf) {
    LOGGER.info("[SyncFluidSlotPacket][SEND] containerId={}, slotIndex={}, fluidStack={}", containerId, slotIndex, fluidStack);
  ...
  public boolean handle(Supplier<NetworkEvent.Context> ctx) {
    ctx.get().enqueueWork(() -> {
      LOGGER.info("[SyncFluidSlotPacket][RECV] containerId={}, slotIndex={}, fluidStack={}", containerId, slotIndex, fluidStack);
      // 客户端：根据containerId和slotIndex更新GUI流体槽内容
      net.minecraft.client.Minecraft mc = net.minecraft.client.Minecraft.getInstance();
      var player = mc != null ? mc.player : null;
      if (player == null) return;
      var menu = player.containerMenu;
      if (menu != null && menu.containerId == this.containerId && menu instanceof kivo.millennium.milltek.container.Device.AbstractDeviceMenu<?> devMenu) {
